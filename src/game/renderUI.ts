...
export function renderUI(gc: GameCtx): void {
  ...
  if (level.water !== undefined && player.air < 1) {
    ctx.save();
    ctx.translate(140, meterY);
    art.drawMeter(ctx, player.air, '#5ec8f0', gc.globalT, 200, 16);
  ...
  if (player.hasWings && !player.rescue) {
    ctx.save();
    ctx.translate(140, meterY);
    art.drawMeter(ctx, player.flyCharge, '#ffcf3d', gc.globalT, 200, 16);
    ctx.translate(-118, 0);
    art.drawStar(ctx, 9, player.flyCharge >= 1 ? 0.6 : 0);
    ctx.restore();
  }
